...
Database::Database()
{
	...
	{*/
		...
		resultFile << "index and data file initialize complete." << endl;
		index.close();
		data.close();
...
void Database::dataFile_find(const int dataAddress, string &value)
{
	...
	value = tmp;
	resultFile << "dataFile_find dataAddress:" << dataAddress << " value:" << value << " tmp:" << tmp << endl;
}
...
int Database::dataFile_add(const string &value)
{
	...
	resultFile << "dataFile_add value:" << value << " dataAddress:" << dataAddress << " next:" << next << endl;
	return dataAddress;
}
...
int Database::indexFile_find(int key, int *indexAddress, int *pos, int *size, int *dataAddress)
{
	...
	while (next > 0)
	{
		...
		do
		{
			...
			resultFile << "[indexFile_find] middle:" << middle << " tmp:" << tmp << endl;
			if (tmp == key)
				begin = end = middle;
			else if (tmp > key)
				end = middle;
		...
		resultFile << "[indexFile_find] key:" << key << " address:" << next << " begin:" << begin << " end:" << end << " middle:" << middle << " tmp:" << tmp << " size:" << *size << endl;
		if (end == *size)
		{
			indexFile.seekg(next + 12 + 8 * (*size - 1));
			indexFile.read(reinterpret_cast<char *>(&last), sizeof(int));
			resultFile << "[indexFile_find] last:" << last << endl;
			if (key >= last)
				begin = *size;
		}
...
{
	resultFile << "indexFile_add key:" << key << " dataAddress:" << dataAddress << " indexAddress:" << indexAddress << " pos:" << pos << " size:" << size << endl;
	if (size == 0)
	{
		int nextEmpty = 0;
...
{
	resultFile << "indexFile_addAndOverflow key:" << key << " dataAddress:" << dataAddress << " indexAddress:" << indexAddress << " pos:" << pos << " size:" << size << endl;
	if (size < scale)
	...
	else
	{
		...
		for (int i = 0; i < rightSize; ++i)
		{
			resultFile << "[indexFile_addAndOverflow] index[" << i << "]:" << index[i] << endl;
			resultFile << "[indexFile_addAndOverflow] data[" << i << "]:" << data[i] << endl;
		}
		indexFile.clear();
		...
		resultFile << "indexFile_addAndOverflow new node firstEmpty:" << firstEmpty << " nextEmpty:" << nextEmpty << endl;
		if (dataAddress > 0)
		{
		...
		else
		{
			...
			resultFile << "[indexFile_addAndOverflow] currentLeaf:" << firstEmpty << " nextLeaf:" << nextLeaf << endl;
		}
		indexFile.seekg(indexAddress);
		indexFile.read(reinterpret_cast<char *>(&parent), sizeof(int));
		if (parent == 0)
		{
			...
			indexFile.write(reinterpret_cast<char *>(&firstEmpty), sizeof(int));
			indexFile.write(reinterpret_cast<char *>(&parentPositon), sizeof(int));
			resultFile << "[indexFile_addAndOverflow] newRoot:" << firstEmpty << " left:" << left << " leftChild:" << leftChild << " right:" << right << " rightChild:" << rightChild << " nextEmpty:" << nextEmpty << endl;
		}
		else
		{
...
void Database::indexFile_delete(const int indexAddress, const int pos, int size)
{
	resultFile << "[indexFile_delete] indexAddress:" << indexAddress << " pos:" << pos << " size:" << size << endl;
	int curKey, curPoint;
...
void Database::indexFile_borrowLeft(const int indexAddress, int size, const int left, int leftSize, int parent, int parentPosition)
{
	resultFile << "[indexFile_borrowLeft] indexAddress:" << indexAddress << " size:" << size << " left:" << left << " leftSize:" << leftSize << " parent:" << parent << " parentPosition:" << parentPosition << endl;
...
void Database::indexFile_borrowRight(const int indexAddress, int size, const int right, int rightSize, int parent, int parentPosition)
{
	resultFile << "[indexFile_borrowRight] indexAddress:" << indexAddress << " size:" << size << " right:" << right << " rightSize:" << rightSize << " parent:" << parent << " parentPosition:" << parentPosition << endl;
	...
	resultFile << "[indexFile_borrowRight] borrowKey:" << borrowRightKey << " borrowValue:" << borrowRightValue << endl;
	...
	for (int i = 0; i < rightSize - 1; ++i)
		resultFile << "[indexFile_borrowRight] index[" << i << "]:" << index[i] << " data[" << i << "]:" << data[i] << endl;
	size += 1;
	indexFile.seekp(indexAddress + 8);
	indexFile.write((char *)&size, sizeof(int));
...
void Database::indexFile_mergeLeft(const int indexAddress, const int size, const int left, int leftSize)
{
	resultFile << "[indexFile_mergeLeft] indexAddress:" << indexAddress << " size:" << size << " left:" << left << " leftSize:" << leftSize << endl;
	int *index = new int[size];
	int *data = new int[size];
	int firstEmpty;
	indexFile.seekg(indexAddress + 12);
...
void Database::indexFile_mergeRight(const int indexAddress, int size, const int right, const int rightSize)
{
	resultFile << "[indexFile_mergeRight] indexAddress:" << indexAddress << " size:" << size << " right:" << right << " rightSize:" << rightSize << endl;
	int firstEmpty;
...
void Database::indexFile_deleteAndUnderflow(const int indexAddress, const int pos, int size)
{
	resultFile << "[indexFile_deleteAndUnderflow] indexAddress:" << indexAddress << " pos:" << pos << " size:" << size << endl;
	indexFile_delete(indexAddress, pos, size);
	size -= 1;
	if (size < (scale + 1) / 2)
	{
		...
		if (parent != 0)
		{
			...
			resultFile << "[indexFile_deleteAndUnderflow] parent:" << parent << " parentPosition:" << parentPosition << " parentSize:" << parentSize << endl;
			if (parentPosition == parentSize)
			{
				...
				else
				{
					...
					indexFile.seekg(4);
					indexFile.read((char *)&firstEmpty, sizeof(int));
					indexFile.seekp(parent);
					indexFile.write((char *)&firstEmpty, sizeof(int));
					indexFile.seekp(indexAddress);
					indexFile.write((char *)&ZERO, sizeof(int));
					indexFile.seekp(0);
					indexFile.write((char *)&indexAddress, sizeof(int));
					indexFile.write((char *)&parent, sizeof(int));
				}
			}
			else
			{
...
{
	resultFile << endl << "UPDATE key:" << key << " value:" << value << endl;
	int indexAddress, pos, size, dataAddress;
	int result = indexFile_find(key, &indexAddress, &pos, &size, &dataAddress);
	if (result != 1)
		return false;
